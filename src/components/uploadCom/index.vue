<template>
  <div class="text-msg-pic-upload">
    ...
    >
      <el-button v-loading="isLoading" size="small" :disabled="disabled" type="primary">{{ buttonLabel }}</el-button>
...
<script>
import { UploadFile, DownloadFileURL } from '@/api/apis'
export default {
  ...
  methods: {
    ...
    filterFileFn(fileList) {
      const filterArr = fileList
        ...
        .map(item => {
          const url = DownloadFileURL({ id: item.response })
          return {
            url: url,
            name: item.name
          }
