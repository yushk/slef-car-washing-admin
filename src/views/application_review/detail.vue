<template>
  <div class="m-4 index-container">
        ...
        <div v-for="(q,i3) in second.questions" :key="q.id" class="mt-2 ml-4">
          <div class="flex flex-row">
            <div class="relative w-2/3">
              ...
              <el-radio-group v-model="applyObj[q.id].reviewValue" class="flex flex-col ml-4" :disabled="approverMap[q.id]">
                <el-radio
            ...
            <div class="w-1/3">
              <FileList v-if="q.uploadFile==='1'" :list="applyObj[q.id].files||[]" :title="q.fileName" />
            </div>
          </div>
        </div>
      </el-card>
    ...
    <div class="flex justify-center mt-2">
      ...
      <el-button v-if="ruleForm.status ==='1'" type="warning" @click="submitForm('1')">保存</el-button>
      <el-button v-if="ruleForm.status ==='1'" type="success" @click="submitForm('2')">提交</el-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
      ...
      GetHomeWork({ id: id }).then(res => {
        if (res) {
          ...
          res.apply_items.forEach((e) => {
            if (e.reviewValue === '') {
              e.reviewValue = e.value
            }
            if (e.reviewUser === '') {
              e.reviewUser = this.userid
            }
    ...
    generateTemplate(data) {
      ...
      const firstObj = {}
      data.forEach(e => {
        this.approverMap[e.id] = e.approver || false
